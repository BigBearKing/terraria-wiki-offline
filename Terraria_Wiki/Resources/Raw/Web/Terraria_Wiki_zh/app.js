...
function refresh() {
    ...
    sounds.forEach(container => {
        ...
        container.title = window.wikiApp.t('Web.ClickToPlay', '点击播放');
        ...
        container.addEventListener('click', function (e) {
            ...
            if (audio.paused) {
                ...
                this.title = window.wikiApp.t('Web.ClickToStop', '点击停止');
            } else {
                audio.pause();
                audio.currentTime = 0;
                this.classList.remove('sound-playing');
                this.title = window.wikiApp.t('Web.ClickToPlay', '点击播放');
            }
        });
    });
...
window.wikiApp.start({ homePage: "Terraria Wiki", headingId: "firstHeading", homeHeadingId: "firstHeading", lastModifiedPrefix: "此页面最后编辑于 ", homePageClass: "rootpage-Terraria_Wiki", openImageLinks: false, sourceUrl: (title) => "https://terraria.wiki.gg/zh/wiki/" + encodeURIComponent(title.replace(/ /g, "_")), refresh });
