...
handlers["BackHome"] = async () => {
    await redirect("Calamity Mod Wiki");
...
async function redirect(title) {
    ...
    document.getElementById("footer-info-lastmod").textContent = "This page was last edited on " + result.lastModified;
    if (title == "Calamity Mod Wiki") {
        document.body.classList.add("rootpage-Calamity_Mod_Wiki");
        document.getElementById("firstHeading").setAttribute("style", "display:none");
    } else {
        document.body.classList.remove("rootpage-Calamity_Mod_Wiki");
    ...
    if (btnOpenSource) {
        btnOpenSource.addEventListener('click', () => {
            ...
            if (aTag && aTag.href && aTag.href.startsWith('http')) {
                ...
                const title = window.pageTitle || "Calamity Mod Wiki";
                targetUrl = "https://calamitymod.wiki.gg/wiki/" + encodeURIComponent(title.replace(/ /g, "_"));
            }
